...
void app_main(void)
{
    ...
    // Set volume to 100% (maximum volume)
    ESP_LOGI(TAG, "[ 1.1 ] Setting volume to 100");
    esp_err_t volume_ret = audio_hal_set_volume(board_handle->audio_hal, 90);
    if (volume_ret == ESP_OK) {
        ESP_LOGI(TAG, "Volume successfully set to 100");
    } else {
        // ESP_LOGW(TAG, "Failed to set volume to 100%, error: %d", volume_ret);
    }
